...
window.addEventListener('keydown', function(e) {
		...
		// Run code for CTRL+SHIFT+L
			...
			if(shortcutlight == 'true'){
				chrome.extension.sendRequest({name: 'automatic'});
			}
		}
}, false);
...
var gracePeriod = 250, lastEvent = null, timeout = null;
	...
	function shadesOff(player) {
		...
		var blackon = $('stefanvdlightareoff1');
			if (blackon) {chrome.extension.sendRequest({name: 'automatic'});}
			else {} // do nothing
		}
	}
	function shadesOn(player) {
		...
		var blackon = $('stefanvdlightareoff1');
			...
			else {chrome.extension.sendRequest({name: 'automatic'});}			
		}
	}
...
if(eyea == 'true'){chrome.extension.sendRequest({name: 'automatic'});}
...
if(typeof excludedDomains == "string") {
	...
	for(var i = 0; i < buf.length; i++)
		if(currenturl == buf[i]){chrome.extension.sendRequest({name: 'automatic'});}
    }
}
}
///////
